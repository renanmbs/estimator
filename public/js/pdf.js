document.addEventListener("DOMContentLoaded", () => {
  ...
  const projectName = data.choices_array?.[0]?.["Project Name"] || "";
  const companyName = data.choices_array?.[0]?.["Company Name"] || "";
  const displayElement = document.getElementById('project-display-name');

  // --- POPULATE HEADER ---
  if (data.choices_array && data.choices_array.length > 0) {
    const firstRow = data.choices_array[0];
    
    const customerDiv = document.getElementById('customer-name-display');
    const projectDiv = document.getElementById('project-name-display');

    if (customerDiv) {
      customerDiv.textContent = firstRow["Company Name"] || "";
    }
    if (projectDiv) {
      // Optional: Add a "Project: " label prefix
      const pName = firstRow["Project Name"];
      projectDiv.textContent = pName ? `Project: ${pName}` : "";
    }
  }
  ...
  const HEADER_MAPPING = {
    "Hole Amount": "Amount of Holes",
    "Hole Size": "Hole Size Ø",
    "Price per Z Clip": "Single Z Clip Price",
    "Total Z Clip Price": "Total Z Clip Price",
    "Notes Status": "Notes"
  };
    ...
    .map(
      row =>
        `<tr>${finalDisplayHeaders
          .map(h => {
            let value = row[h] != null ? row[h] : "";

            // // Check if the header is one of the price columns
            // if (h === "Price per Z Clip" || h === "Total Z Clip Price") {
            //   // Only add $ if there is actually a value present
            //   if (value !== "") {
  ...
  if (totalDiv) {
  totalDiv.innerHTML = `ORDER TOTAL: <span class="price-highlight">$${data.price_per_group}</span>`;
}
  ...
  function generateSVG(template, row) {
    ...
    // 4. Hole Size (New logic to replace {hole_diameter})
    const holeSize = row["Hole Size"] ? row["Hole Size"].toString() : "";
    const formattedHoleSize = holeSize.startsWith("0.")
      ? holeSize.substring(1)
      : holeSize;

    // Replace {hole_diameter} in your SVG template
    svg = svg.replace(/{hole_diameter}/g, formattedHoleSize);

    // 5. Hole Quantity
    const holeAmount = parseInt(row["Hole Amount"], 10);
    if (holeAmount > 0) {
      svg = svg.replace(/{holes}/g, holeAmount);
    } else {
      // If there are no holes, remove the diameter callout entirely
      // This regex looks for the diameter symbol (Ø) and the bracketed diameter tag
      const noHolesRegex = /Ø{hole_diameter}\s+THRU, <tspan[^>]*>\{holes\}X<\/tspan>/g;

      // If the regex doesn't match your specific SVG structure, 
      // simple replacement as a fallback:
      if (svg.match(noHolesRegex)) {
        svg = svg.replace(noHolesRegex, "No Holes");
      } else {
        svg = svg.replace(/{holes}/g, "0");
        svg = svg.replace(/Ø{hole_diameter}/g, "No Holes");
      }
    }
  ...
  function buildRowTableData(row, weight) {
    ...
    const headers = [
      ...
      "Lead In", "Price per Z Clip", "Total Z Clip Price", "Weight", "Material", "Finish", "Customer Notes"
    ];
    ...
    const values = [
      ...
      row?.["Price per Z Clip"] || "",
      row?.["Total Z Clip Price"] || "",
      `${weight} lbs`,
      "Aluminum 6063",
      "Mill",
      row?.["Additional Notes"] || ""
    ];
    const filtered = headers.map((h, i) => {
      let val = values[i];

      // Add $ to specific pricing headers if the value is not empty
      // if ((h === "Price per Z Clip" || h === "Total Z Clip Price") && val !== "") {
      //   // Formats to 2 decimal places and adds the $ sign
      //   val = `$${parseFloat(val).toFixed(2)}`;
      // }

      return { header: h, value: val };
    })
      .filter(col => col.value !== "" && col.value != null);
  ...
  downloadBtn.addEventListener("click", () => {
    ...
    logo.onload = () => {
      ...
      const summaryHeaders = finalDisplayHeaders;

      // Map display names
      const formattedHeaders = summaryHeaders.map(h => HEADER_MAPPING[h] || h);

      // Build body rows in same order with currency formatting
      const bodyRows = data.choices_array.map(r =>
        summaryHeaders.map(h => {
          let val = r[h] != null ? r[h] : "";

          // Check if this header is a price column
          // if ((h === "Price per Z Clip" || h === "Total Z Clip Price") && val !== "") {
          //   // Convert to number, format to 2 decimals, and add $
